...
public class CandleDrawing implements IDrawing {
    ...
    private float[] previouCandleLineBuffer = new float[8];
    private float[] previouCandleRectBuffer = new float[4];
    @Override
...
//        }
        ...
        for (int i = minIndex; i < maxIndex; i++) {
            ...
            if (render.isHighlight()) {
                ...
                Log.d(TAG,"highlightPoint0 : "+highlightPoint[0]+"\ncandleRectBuffer[0] : "+candleRectBuffer[0]+"\ncandleRectBuffer[2] : "+candleRectBuffer[2]);
                if (candleRectBuffer[0] <= highlightPoint[0] && highlightPoint[0] <= candleRectBuffer[2]) {
                   ...
                   // highlightPoint[1] = (candleRectBuffer[1] + candleRectBuffer[3]) / 2;
                    if (entry.getOpen()>entry.getClose()){
                        highlightPoint[1] = candleLineBuffer[5];
                    }else {
                        highlightPoint[1] = candleLineBuffer[3];
                    }
                    entrySet.setHighlightIndex(i);
                    Log.d(TAG,"entrySet.getHighlightIndex() : "+entrySet.getHighlightIndex());
                }else if (highlightPoint[0] >= previouCandleRectBuffer[2]&&highlightPoint[0]<=candleRectBuffer[0]){//解决高亮线出现在两个蜡烛图之间问题
                    Log.d(TAG," previouhighlightPoint[0] candle : "+highlightPoint[0]);
                    highlightPoint[0] = candleLineBuffer[0];
                    if (entry.getOpen()>entry.getClose()){
                        highlightPoint[1] = candleLineBuffer[5];
                    }else {
            ...
            previouCandleLineBuffer[0] = candleLineBuffer[0];
            previouCandleRectBuffer[0] = candleRectBuffer[0];
            previouCandleRectBuffer[2] = candleRectBuffer[2];
        }
