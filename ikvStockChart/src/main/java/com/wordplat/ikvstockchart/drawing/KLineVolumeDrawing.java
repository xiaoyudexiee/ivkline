...
public class KLineVolumeDrawing implements IDrawing {
    ...
    private Paint ma20Paint; // 成交量的20日平均线画笔
    ...
    public void computePoint(int minIndex, int maxIndex, int currentIndex) {
        ...
        if (currentIndex < maxIndex - 1) {
            ...
            ma20Buffer[i * 4 + 0] = 0;
            ma20Buffer[i * 4 + 1] = (float) entry.getVolumeMa20();
            ma20Buffer[i * 4 + 2] = 0;
            ma20Buffer[i * 4 + 3] = (float) entrySet.getEntryList().get(currentIndex + 1).getVolumeMa20();
        }
    }
    ...
    public void onComputeOver(Canvas canvas, int minIndex, int maxIndex, float minY, float maxY) {
        ...
        for (int i = minIndex; i < maxIndex; i++) {
            ...
            if (render.isHighlight()) {
                ...
                if (xRectBuffer[0] <= highlightPoint[0] && highlightPoint[0] <= xRectBuffer[2]) {
                    Log.d("KLineHighlight", "candleBuffer[1] : " + candleBuffer[1]);
                    highlightPoint[1] = candleBuffer[1];
        ...
        render.mapPoints(null, ma20Buffer);
        ...
        for (int i = 0 ; i < count ; i = i + 4) {
            ...
            ma20Buffer[i + 0] = xPointBuffer[i + 0];
            ma20Buffer[i + 2] = xPointBuffer[i + 2];
        }
