...
public class YAxisTextMarkerView implements IMarkerView {
    ...
    private HightLightStyle hightLightStyle;
    ...
    @Override
    public void onInitMarkerView(RectF contentRect, AbstractRender render, Context context) {
        this.contentRect.set(contentRect);
        this.render = render;
        mContext=context;
        sizeColor = render.getSizeColor();

        hightLightStyle = sizeColor.getHightLightStyle();

        if (markerTextPaint == null) {
            markerTextPaint = new Paint(Paint.ANTI_ALIAS_FLAG);

            markerTextPaint.setTextAlign(hightLightStyle ==HightLightStyle.IN? Paint.Align.CENTER:Paint.Align.LEFT);
        }
        ...
        if (markerBorderPaint == null) {
            ...
            markerBorderPaint.setStyle(hightLightStyle ==HightLightStyle.IN?Paint.Style.STROKE: Paint.Style.FILL);
        }
        ...
        Log.d(TAG,"yMarkerAlign1 = "+yMarkerAlign);
    }
    ...
    public void onDrawMarkerView(Canvas canvas, float highlightPointX, float highlightPointY) {
        if (contentRect.top < highlightPointY && highlightPointY < contentRect.bottom) {
            ...
            if (hightLightStyle == HightLightStyle.IN) {
          ...
          /*  markerInsets.left = contentRect.right;
            markerInsets.top = highlightPointY + inset;
            markerInsets.right = markerInsets.left + width - inset * 2;
            markerInsets.bottom = markerInsets.top + height - inset * 2;*/

            Log.d(TAG,"yMarkerAlign = "+yMarkerAlign);
            markerInsets.top = highlightPointY + inset;
            markerInsets.bottom = markerInsets.top + height - inset * 2;
            if (yMarkerAlign ==YMarkerAlign.RIGHT) {
                markerInsets.left = contentRect.right;
                markerInsets.right = markerInsets.left + width - inset * 2;
            }else if (yMarkerAlign == YMarkerAlign.LEFT){
                markerInsets.left = contentRect.left ;
            }

            canvas.clipRect(markerInsets, Region.Op.XOR);
            markerTextPaint.setColor(sizeColor.getMarkerBorderColor());
            canvas.drawRect(markerInsets, markerTextPaint);
            markerTextPaint.setColor(sizeColor.getMarkerTextColor());
            canvas.drawText(value,
                    yMarkerAlign ==YMarkerAlign.RIGHT?markerInsets.left+ ViewUtils.dpTopx(mContext,2):ViewUtils.dpTopx(mContext,2),
                    (markerInsets.top + markerInsets.bottom - fontMetrics.top - fontMetrics.bottom) / 2,
                    markerTextPaint);
