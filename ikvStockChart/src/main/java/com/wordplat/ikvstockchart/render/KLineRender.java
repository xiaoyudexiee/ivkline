...
import com.wordplat.ikvstockchart.drawing.FenshiDrawing;
...
public class KLineRender extends AbstractRender {
    ...
    private final FenshiDrawing fenshiDrawing = new FenshiDrawing();
...
//        kLineDrawingList.add(maDrawing);
        kLineDrawingList.add(fenshiDrawing);
    ...
    public void addDrawing(IDrawing drawing) {
        if (!kLineDrawingList.contains(maDrawing)&&drawing.equals(maDrawing)) {
            kLineDrawingList.remove(bollDrawing);
//            kLineDrawingList.remove(fenshiDrawing);
            kLineDrawingList.add(drawing);
        }
        if (!kLineDrawingList.contains(bollDrawing)&&drawing.equals(bollDrawing)){
            kLineDrawingList.remove(maDrawing);
//            kLineDrawingList.remove(fenshiDrawing);
            kLineDrawingList.add(drawing);
        }

        if (!kLineDrawingList.contains(fenshiDrawing)&&drawing.equals(fenshiDrawing)){
            kLineDrawingList.remove(maDrawing);
            kLineDrawingList.remove(bollDrawing);
            kLineDrawingList.add(drawing);
        }

        if (!kLineDrawingList.contains(candleDrawing)&&drawing.equals(candleDrawing)){
            kLineDrawingList.remove(fenshiDrawing);
            kLineDrawingList.add(candleDrawing);
            kLineDrawingList.add(maDrawing);
        }
    ...
    public void addCandleDrawing(IDrawing drawing,int mainType){
        if (!kLineDrawingList.contains(candleDrawing)&&drawing.equals(candleDrawing)){
            kLineDrawingList.remove(fenshiDrawing);
            kLineDrawingList.add(candleDrawing);
            if (mainType == 1&&!kLineDrawingList.contains(maDrawing)) {
                kLineDrawingList.add(maDrawing);
            }else if (mainType ==2&&!kLineDrawingList.contains(bollDrawing)){
    ...
    public MADrawing getMaDrawing() {
        return maDrawing;
    }

    public FenshiDrawing getFenshiDrawing() {
        return fenshiDrawing;
    }

    public BOLLDrawing getBollDrawing() {
        return bollDrawing;
    }

    public CandleDrawing getCandleDrawing() {
        return candleDrawing;
    }
    ...
    public void onViewRect(RectF viewRect) {
        ...
        initDrawingList(kLineRect, kLineDrawingList,context);
        ...
        for (StockIndex stockIndex : stockIndexList) {
            if (stockIndex.isEnable()) {
                ...
                initDrawingList(stockIndex.getRect(), stockIndex.getDrawingList(),context);
            }
        }
    }
    ...
    private void initDrawingList(RectF rect, List<IDrawing> drawingList,Context context) {
        for (IDrawing drawing : drawingList) {
            drawing.onInit(rect, this,context);
        }
    }
